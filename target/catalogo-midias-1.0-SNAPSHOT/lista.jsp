...
<%
    // --- parâmetros de filtro/ordenação ---
    String q      = request.getParameter("q");
    String tipo   = request.getParameter("tipo"); // FILME | LIVRO
    String sort   = request.getParameter("sort");
    String dirRaw = request.getParameter("dir");  // Crescente | Decrescente | ASC | DESC
    ...
    java.util.List<String> cols = java.util.Arrays.asList("id","titulo","autor","tipo","ano","nota");
    if (sort == null || !cols.contains(sort)) sort = "id";

    String dir, dirLabel;
    if ("Decrescente".equalsIgnoreCase(dirRaw) || "DESC".equalsIgnoreCase(dirRaw)) {
        dir = "DESC"; dirLabel = "Decrescente";
    } else {
        dir = "ASC";  dirLabel = "Crescente";
    }

    br.projeto.dao.MidiaDAO dao = new br.projeto.dao.MidiaDAO();

    // total primeiro, para poder ajustar a página se necessário
    String tipoFiltro = (tipo != null && !tipo.isEmpty()) ? tipo : null;
    int total = dao.contar(q, tipoFiltro);
    int totalPages = (int) Math.ceil(total / (double) pageSize);
    if (totalPages == 0) totalPages = 1; // evita divisão por zero na UI
    if (pageNumber > totalPages) pageNumber = totalPages;

    int offset = (pageNumber - 1) * pageSize;

    java.util.List<br.projeto.model.Midia> midias =
        dao.listar(q, tipoFiltro, sort, dir, offset, pageSize);
...
<div class="card">
  ...
  <form method="get" action="${pageContext.request.contextPath}/lista.jsp"
    ...
    <select class="select" name="dir" style="width:180px">
      <option value="Crescente"   <c:if test="${param.dir=='Crescente' || param.dir=='ASC' || empty param.dir}">selected</c:if>   >Crescente</option>
      <option value="Decrescente" <c:if test="${param.dir=='Decrescente' || param.dir=='DESC'}">selected</c:if> >Decrescente</option>
    </select>
  ...
  <table class="table">
    ...
    <c:forEach var="m" items="${midias}">
      <tr>
        ...
        <td><a href="${pageContext.request.contextPath}/detalhe.jsp?id=${m.id}"><c:out value="${m.titulo}"/></a></td>
        <td><c:out value="${m.autor}"/></td>
        <td><c:out value="${m.tipo}"/></td>
        <td><c:out value="${m.ano}"/></td>
        <td>
          <c:choose>
            <c:when test="${m.nota != null}">
              <fmt:formatNumber value="${m.nota}" maxFractionDigits="1" />
            </c:when>
  ...
  <c:if test="${totalPages > 1}">
    <c:url var="urlFirst" value="/lista.jsp">
      ...
      <c:param name="dir"  value="${dirLabel}"/>
